### Запрос 1: Получение штампов для пользователя
-- Получение штампов для пользователя с ID 2976
CREATE TEMP TABLE temp_person_stamp_values AS
SELECT "stamp"
FROM "person_stamp"
WHERE "person" = 2976;


### Запрос 2: Получение групп текущего пользователя
-- Получение групп текущего пользователя с ID 2976
CREATE TEMP TABLE temp_cur_user_groups AS
...
### Запрос 3: Получение данных ответственных исполнителей
-- Получение данных ответственных исполнителей
CREATE TEMP TABLE responsible_executors AS
SELECT 
...
WHERE EXISTS (
    ...
    WHERE ptf."id" = responsible_executor."access_object_id"
      AND (ptf."security_stamp" IS NULL OR ptf."security_stamp" IN (SELECT "stamp" FROM temp_person_stamp_values))
)
AND EXISTS (
    SELECT 1
    FROM "f_dp_rkkbase_read" r
    WHERE r."group_id" IN (SELECT "parent_group_id" FROM temp_cur_user_groups)
    AND r."object_id" = responsible_executor."access_object_id"
);


### Запрос 4: Основные данные резолюций
-- Получение основных данных резолюций
SELECT 
    resolution."id",
    resolution."id_type",
...
Теперь каждый из этих запросов можно выполнять по очереди. Вначале создаются временные таблицы для хранения промежуточных данных, а затем основной запрос используе
